...
import { deepDebuggerPrefix } from './deepDebug';
...
export function makeBinConfig(cfg, nodePath, hookPath) {
    ...
    cfg.args = cfg.args.concat(Array(
        deepDebuggerPrefix + "nodejs-path", nodePath,
        deepDebuggerPrefix + "binary-hook", hookPath,
        deepDebuggerPrefix + "session-name", '"' + cfg.name + ' (binary extensions)"',
        ));
}
